...
const Navbar = ({ scrolled }: { scrolled: boolean }) => {
  ...
  return (
    <nav
      className={`flex w-full transition-all duration-300 z-30 ${
        scrolled && "backdrop-blur-md shadow-lg"
      }`}
    >
      <div className="mx-auto flex w-full max-w-screen-2xl items-center justify-between gap-4 px-4 py-4 md:px-8">
        <div
          className="group relative flex flex-row items-start gap-4 cursor-pointer"
          onClick={() => {
